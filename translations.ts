export const translations = {
  ...
  description: { en: 'Convert Slovenian UPN QR codes into EPC format for payment apps like Revolut.', sl: 'Pretvorite slovenske UPN QR kode v EPC format za plačilne aplikacije, kot je Revolut.' },
  ...
  errorDecode: {en: 'Could not decode QR code from the selected image.', sl: 'QR kode iz izbrane slike ni bilo mogoče prebrati.'},
  ...
  scanTipsTitle: { en: 'Tips for better scanning:', sl: 'Nasveti za boljše skeniranje:' },
  ...
  ERROR_INVALID_FORMAT: { 
    ...
    sl: (lines: number) => `Neveljavna oblika UPN QR kode. Pričakovane so bile vsaj 20 vrstic, ki se začnejo z UPNQR, najdenih pa je bilo ${lines}.`
  },
  ...
  howToPayStep1: { en: 'Use "Download QR" or "Share QR Image" to transfer the code to another device (e.g., a computer).', sl: 'Uporabite "Prenesi QR" ali "Deli sliko QR", da prenesete kodo na drugo napravo (npr. računalnik).'},
  ...
  infoWhatItDoesText: { en: 'This tool converts Slovenian Universal Payment Order (UPN) QR codes into European Payments Council (EPC) QR codes. This allows you to pay Slovenian bills using modern European banking apps like Revolut, which don\'t natively support the UPN format.', sl: 'To orodje pretvori QR kode slovenskega Univerzalnega plačilnega naloga (UPN) v QR kode Evropskega plačilnega sveta (EPC). To omogoča plačevanje slovenskih računov z modernimi evropskimi bančnimi aplikacijami, kot je Revolut, ki izvorno ne podpirajo formata UPN.' },
  ...
  infoStep2: { en: 'The app instantly generates a standard EPC QR code.', sl: 'Aplikacija takoj ustvari standardno EPC QR kodo.'},
  ...
  blogSection2Title: { en: 'Why Are Neobanks So Popular?', sl: 'Zakaj so neobanke tako priljubljene?'},
  ...
  blogSection3Title: { en: 'The Solution', sl: 'Rešitev'},
  blogSection3P1: { en: 'This converter does one thing: it translates UPN codes into EPC codes. You scan the Slovenian UPN code with this app, and it generates a new EPC code that works with any SEPA-compliant banking app. The process is straightforward—scan the original code, get the converted code, then scan that with your banking app to make the payment. No fees, no hassle.', sl: 'Ta pretvornik opravi eno stvar: prevede UPN kode v EPC kode. Z aplikacijo skenirate slovensko UPN kodo in ta ustvari novo EPC kodo, ki deluje z vsako bančno aplikacijo, ki podpira SEPA standard. Postopek je preprost—skenirate izvirno kodo, dobite pretvorjeno kodo, nato to kodo skenirate s svojo bančno aplikacijo za plačilo. Brez provizij, brez zapletov.' },
